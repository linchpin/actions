...
`php_version` sets the runner's PHP *and* the wp-env container's. It must be at
least the plugin's `Requires PHP`, or WordPress refuses to activate it and the
action fails with `No plugins activated`.
